...
@hydra.main(version_base=None, config_path="../../config/risknet", config_name="train")
def train(cfg: RiskNetConfig) -> None:
    """Trains the RiskNet model."""
    with open("config.yaml", "w") as fh:
        OmegaConf.save(cfg, f=fh)
    ...
    tokenizer = layers.StringLookup(mask_token="")
    tokenizer.adapt(build_vocab_from_patients(train_pos + train_neg))
    ...
    train_gen = RiskNetBatchGenerator(
        cfg.model.batch_size,
        cfg.model.max_sequence_length,
        tokenizer=risknet.tokenizer,
    ...
    risknet.fit(
        train_gen.flow(train_pos, train_neg, shuffle=True, seed=cfg.preprocess.seed),
        epochs=cfg.model.epochs,
        validation_data=val_gen.flow(test_pos, test_neg, shuffle=False),
        callbacks=[early_stopping]
    )

    risknet.save("risknet.keras")

    log.info("Evaluating model...")

    eval_batch_generator = RiskNetBatchGenerator(
    ...
    eval_scores = get_eval_scores(
        risknet, eval_batch_generator.flow(test_pos, test_neg), cfg.data.month_endpoints
    )
